    ...
    <div>
      ...
      {cart.map(item=> (
        <div className="d-flex align-items-center border rounded p-2 mb-2" key={item._id}>
          <img src={item.image || 'https://picsum.photos/80'} style={{width:80,height:60,objectFit:'cover'}} />
          <div className="ms-3 flex-grow-1">
            <h6 className="mb-1">{item.title}</h6>
            <small>₹{item.price}</small>
          </div>
          <div className="d-flex align-items-center gap-2">
            <input type="number" value={item.qty} min={1} onChange={e=> updateQty(item._id, Math.max(1, parseInt(e.target.value)||1))} style={{width:70}} className="form-control form-control-sm" />
      ...
      <div className="mt-3">
        ...
        <div className="input-group mb-2" style={{maxWidth:360}}>
          <input className="form-control" value={code} onChange={e=> setCode(e.target.value.toUpperCase())} placeholder="Coupon code" />
          <button className="btn btn-outline-secondary" onClick={apply}>Apply</button>
        </div>
        {msg && <div className="mb-2">{msg}</div>}
        {coupon && <div className="mb-2">Coupon <b>{coupon.code}</b> - {coupon.discountPercent}%</div>}
